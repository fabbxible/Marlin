# Creality CV-01 with Ender-3 12864 LCD support with a lot of changes

Most of the changes are very crude.

## Issues with original firmware:
1. No display.
2. Only one file can be in sd card in a time.
3. Laser engraver starts running immediately when startup.
4. No way to change print speed when running.
5. The only way to re-run is to power cycle.

## Main Features:
1. Add Ender-3 LCD Support to edit configurations and select files.
2. Add Speed Control when running.
3. Hide unused settings (eg. z axis).
4. Allow folder accesss in sd card.
5. Enable M808 repeat marker.

## To use repeat marker:
1. Take the gcode generated by CrealityLaser software.
